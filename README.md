...
### 🔧 两级数据管理系统 (动态优先)
灵活的博物馆数据加载机制，优先远程、静态回退，已移除对单体 `museums-data.js` 的运行时依赖：

- **Tier 2 - 远程存储**：云端 KV 存储中的博物馆数据，用于开发调试和快速迭代（优先）
- **Tier 1 - 静态文件**：单个博物馆的静态 JSON 文件 (`/museums/{museum-id}.json`)，用于稳定发布和回退
- **元数据**：`museums-meta.js` 提供首页轻量列表
