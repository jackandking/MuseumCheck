...
description: '博物馆镇馆之宝查找Skill - 从百度百科等权威媒体获取博物馆的3个镇馆之宝（馆藏精品），包括名称和高质量图片URL。**仅输出结果，不写入任何文件**。'
---
...
> ⚠️ **重要：此Skill仅输出查找结果，不会写入任何文件。开发者需自行决定如何使用输出数据。**
...
✅ 每个鍇馆之宝都有有效的图片URL  
✅ 所有图片URL都经过验证和访问测试  
✅ 每个鍇馆之宝的图片质量评分 >= 70/100  
✅ 结果已输出为JSON格式供开发者参考  
✅ 包含替代图片选项供开发者选择  
✅ 完整的处理日志和质量报告

> ℹ️ **注意：此Skill不会自动写入文件，仅输出结果供开发者自行使用。**  
